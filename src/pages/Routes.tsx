    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Recent Visits */}
                <div>
                    <h3 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-3">Recent Visits</h3>
                    <div className="space-y-2">
                        {visits.length === 0 ? (
                            <div className="glass rounded-xl p-8 text-center text-surface-400 text-sm">No visits yet</div>
                        ) : (
                            visits.map((v) => {
                                    ...
                                    <div key={v.id} className="glass rounded-xl p-3 card-hover">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-medium truncate text-surface-400">{prospect?.prospectname || 'Unknown'}</p>
                ...
                {/* Travel Records */}
                <div>
                    <h3 className="text-sm font-semibold text-surface-500 uppercase tracking-wider mb-3">Travel Records</h3>
                    <div className="space-y-2">
                        {travelRecords.length === 0 ? (
                            <div className="glass rounded-xl p-8 text-center text-surface-400 text-sm">No travel records yet</div>
                        ) : (
                            travelRecords.map((t) => (
                                <div key={t.id} className="glass rounded-xl p-3 card-hover">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium truncate text-surface-400">{t.route_name || `Route #${t.route_id}`}</p>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="glass rounded-2xl p-6 w-full max-w-md animate-slide-up">
                        <div className="flex justify-between mb-5">
                            <h3 className="text-lg font-semibold text-surface-500">Log Visit</h3>
                            <button onClick={() => setShowVisitModal(false)} className="btn-ghost p-1.5"><X className="h-5 w-5" /></button>
                        </div>
                        <div className="space-y-3">
                            <div>
                                <label className="text-xs text-surface-500 mb-1 block">Prospect</label>
            ...
            {showTravelModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="glass rounded-2xl p-6 w-full max-w-md animate-slide-up">
                        <div className="flex justify-between mb-5">
                            <h3 className="text-lg font-semibold text-surface-500">Log Travel</h3>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <label className="text-sm text-surface-500">Ideal route</label>
                            </div>
                            <div>
