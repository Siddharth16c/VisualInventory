            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="glass rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-surface-400">Revenue</span>
                    </div>
                ...
                <div className="glass rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-surface-400">Costs</span>
                    </div>
                ...
                <div className="glass rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-surface-400">Profit</span>
                    </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-surface-700">
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Date</th>
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Type</th>
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Business</th>
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Description</th>
                                <th className="text-right px-4 py-2 text-xs text-surface-400 uppercase">Amount</th>
                        ...
                        <tbody>
                            {monthCosts.length === 0 ? (
                                ...
                                monthCosts.map((c) => (
                                    <tr key={c.id} className="border-b border-surface-800 hover:bg-surface-800/50">
                                        <td className="px-4 py-2">{c.date}</td>
                                        <td className="px-4 py-2"><span className="badge-info">{c.cost_type}</span></td>
                                        <td className="px-4 py-2 text-surface-400">{c.business_type}</td>
                                        <td className="px-4 py-2 text-surface-400">{c.description}</td>
                                        <td className="px-4 py-2 text-right font-semibold text-red-400">₹{c.amount.toFixed(2)}</td>
            ...
            <div className="glass rounded-xl overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-surface-700">
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">ID</th>
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Customer</th>
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Date</th>
                                <th className="text-left px-4 py-2 text-xs text-surface-400 uppercase">Status</th>
                                <th className="text-right px-4 py-2 text-xs text-surface-400 uppercase">Total</th>
                                <th className="text-right px-4 py-2 text-xs text-surface-400 uppercase">Due</th>
                            </tr>
                        </thead>
                        <tbody>
                            {monthOrders.length === 0 ? (
                                ...
                                monthOrders.map((o) => (
                                    <tr key={o.id} className="border-b border-surface-800 hover:bg-surface-800/50">
                                        <td className="px-4 py-2 font-medium">#{o.id}</td>
                                        <td className="px-4 py-2">{o.prospect_name}</td>
                                        <td className="px-4 py-2 text-surface-400">{new Date(o.order_date).toLocaleDateString('en-IN')}</td>
                                        <td className="px-4 py-2">
                                            <span className={o.status === 'pending' ? 'badge-warning' : o.status === 'delivered' ? 'badge-success' : 'badge-info'}>{o.status}</span>
                                        </td>
                                        <td className="px-4 py-2 text-right font-semibold text-emerald-400">₹{o.grand_total.toFixed(2)}</td>
                                        <td className="px-4 py-2 text-right text-amber-400">₹{o.due_amount.toFixed(2)}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
