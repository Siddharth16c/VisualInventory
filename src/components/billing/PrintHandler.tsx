...
export default function PrintHandler({ order, items, onClose }: PrintHandlerProps) {
    ...
    const handleRawBTPdf = async () => {
        try {
            ...
            items.forEach((item) => {
                doc.setFontSize(9);
                doc.text(item.product.product_name.substring(0, 20), 2, y);
                y += lh;
                doc.setFontSize(8);
    ...
    const handleShareInvoice = async () => {
        try {
            ...
            items.forEach((item, idx) => {
                doc.text(`${idx + 1}. ${item.product.product_name} — ${item.qty} x Rs.${item.unit_price.toFixed(2)} = Rs.${(item.qty * item.unit_price - item.discount).toFixed(2)}`, 20, y);
                y += 8;
            });
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/70 flex flex-col">
            {/* Toolbar */}
            <div className="glass flex items-center justify-between px-4 py-3 border-b border-surface-700">
                <h3 className="text-sm font-semibold">Invoice #{order.id}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center gap-8">
                <div className="shadow-2xl rounded-lg overflow-hidden">
                    <InvoiceA4 ref={a4Ref} order={order} items={items} />
                </div>
                <div className="shadow-2xl rounded-lg overflow-hidden">
                    <InvoiceThermal ref={thermalRef} order={order} items={items} />
                </div>
            </div>
        </div>
    );
